...
class CreateAppoinmentController {
  ...
  Future<bool> onCreateAppointment(
      ...
      DateTime time_metting}) async {
    bool result = false;
    int countError = 0;
    _errController.sink.add('Ok');
    if (address_appointment == null || address_appointment == '') {
      _errController.sink.add("Vui điền địa chỉ.");
      countError++;
    }
    if (document_id_user_handing == null) {
      _errController.sink.add("Vui lòng chọn người xử lí.");
      countError++;
    }
    if (custommer_out_the_system == null) {
      _errController.sink.add("Vui lòng chọn khách hàng.");
      countError++;
    }
    if (document_id_product == null) {
      _errController.sink.add("Vui lòng chọn sản phẩm");
      countError++;
    }
    if (document_id_custommer == null) {
      _errController.sink.add(
          "Không lấy được thông thin người lên lịch hẹn. \nVui lòng thử lại sau !");
      countError++;
    }
    if (time_metting == null) {
      _errController.sink.add("Vui chọn thời gian gặp mặt.");
      countError++;
    }
    if (countError == 0) {
      var api = HttpApi();
      ...
      if (document_id_custommer_out_the_system != null) {
        ...
        _errController.sink.add(
            "Quá trình đăng thông tin khách hàng có lỗi !. \n Vui lòng thử lại sau !");
        countError++;
      }
      return result;
    }
